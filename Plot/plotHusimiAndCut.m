...
if plotOption
    ...
    if showLegend
        ...
        l.FontSize = 10;
        text(min(binsO1),max(binsO2)*0.8,['n_{Th} = ' num2str(nTherm,'%.6f') ...
            ' \pm ' num2str(nThermErr,'%.6f') char(10) ' n_{Coh} = ' num2str(nCoherent,'%.6f')...
            ' \pm '  num2str(nCohErr,'%.6f') char(10) ' C = ' num2str(Coherence,'%.6f')  ' \pm ' num2str(CoherenceErr,'%.6f') ],'Color','g');
    end
    xL = ax.XLim;
    BarPos = get(hBar,'Position');
    set(hBar,'Position',BarPos+[0.03 0 0 -0.1]);
    hBar.FontSize = 25;
    savefig([filename '-nbins-' num2str(nBinsA) '-fitMethod-' fitMethod '-Legend-' num2str(showLegend) '-MCError-' num2str(monteCarloError) '-Husimi.fig']);
    print([filename '-nbins-' num2str(nBinsA) '-fitMethod-' fitMethod '-Legend-' num2str(showLegend) '-MCError-' num2str(monteCarloError) '-Husimi.png'],'-dpng','-r300');
    clf();
