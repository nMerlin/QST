function [ HusFunc,xx,yy ] = CreateDisplacedThermalHusimiPhaseAveragedTheory( n, WLength, WRes, r,varargin)
...
xfit = 1/sqrt(2)*sqrt(xx.^2+yy.^2);
HusFunc = (pi*(n+1))^-1 *exp(-(x.^2 + 0.5*r^2)/(n+1)) .* besseli(0,2*x*sqrt(0.5*r^2)/(n+1));
